...
// the envelope stays google.protobuf.Any end to end. Message types are
// imported by proto path (the form protoc-gen-python emits); the build's
// import reroot (scripts/fixup_gen_imports.py) maps them onto the gen package.
...
func (r *pyRefs) emitImports(g *protogen.GeneratedFile) {
	g.P("from typing import Optional, Protocol")
	g.P()
	g.P("import angzarr_router_ffi as ", pyAz)
	g.P("from ", pyTypesModule[:strings.LastIndex(pyTypesModule, ".")], " import ", pyTypesModule[strings.LastIndex(pyTypesModule, ".")+1:], " as ", pyTypes)
	if r.needCmdHdr {
		g.P("from ", pyCmdHdrModule[:strings.LastIndex(pyCmdHdrModule, ".")], " import ", pyCmdHdrModule[strings.LastIndex(pyCmdHdrModule, ".")+1:], " as ", pyCmdH)
	}
	if r.needPM {
		g.P("from ", pyPMModule[:strings.LastIndex(pyPMModule, ".")], " import ", pyPMModule[strings.LastIndex(pyPMModule, ".")+1:], " as ", pyPM)
	}
	for _, p := range r.order {
		pkg, module := pyModule(p)
		alias := r.alias[p]
		if pkg == "" {
			g.P("import ", module, " as ", alias)
		} else {
			g.P("from ", pkg, " import ", module, " as ", alias)
		}
	}
	g.P()
}
