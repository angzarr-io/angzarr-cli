...
// fromPkg is the importing file's dotted package (its proto directory), used to
// emit position-independent relative imports for the generated message modules.
func (r *pyRefs) emitImports(g *protogen.GeneratedFile, fromPkg string) {
	rel := func(target string) (string, string) { return pyRel(fromPkg, target) }
	g.P("from typing import Optional, Protocol")
	g.P()
	g.P("import angzarr_router_ffi as ", pyAz)
	p, m := rel(pyTypesModule)
	g.P("from ", p, " import ", m, " as ", pyTypes)
	if r.needCmdHdr {
		p, m := rel(pyCmdHdrModule)
...
func (e pyEmitter) EmitComponent(g *protogen.GeneratedFile, file *protogen.File, s *Service) error {
	...
	g.P()
	refs.emitImports(g, pyFilePkg(file))
